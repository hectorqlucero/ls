...
(def security-comments-1
  "Only <strong> administrators </strong> can access this option!!!")
...
(defn build-grid-view
  ...
  (let [folder (:folder options)
    ...
    (str
     ...
     "[contactos.models.form :refer [form build-hidden-field build-field build-select build-radio build-modal-buttons build-textarea]]\n"
     "[contactos.models.grid :refer [build-grid build-modal modal-script]]))\n\n"
     "(defn " folder "-view\n"
     "[title rows]\n"
